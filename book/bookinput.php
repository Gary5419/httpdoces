...
if($member_id != ""){
	require '../class_mypage/db_extension.php';
	$dbname = "pokedb";
	$table = "members";
	$columns = "*";
	#$postfix = "where session_id = '{$session_id}' and id ='{$member_id}'  ";
	$postfix = "where id ='{$member_id}'  ";
	$result = DbEx::select($dbname, $table, $columns, $postfix);
	$login_flag = false;
	if( count($result) > 0){
		$login_flag = true;
		$data_rsv = $result[0];
		$tmp = explode("-",$data_rsv['birthday']);
		$data_rsv['birthday_y'] = $tmp[0];
		$data_rsv['birthday_m'] = $tmp[1];
		$data_rsv['birthday_d'] = $tmp[2];
		
		$tmp = explode("-",$data_rsv['tel1']);
		$data_rsv['tel1_1'] = $tmp[0];
		$data_rsv['tel1_2'] = $tmp[1];
		$data_rsv['tel1_3'] = $tmp[2];
		
		$tmp = explode("-",$data_rsv['zip']);
		$data_rsv['zip_1'] = $tmp[0];
		$data_rsv['zip_2'] = $tmp[1];
		$m_pref = array( 
		1 => '北海道',2 => '青森県',3 => '岩手県',4 => '宮城県',5 => '秋田県',6 => '山形県',7 => '福島県',8 => '茨城県',9 => '栃木県',10 => '群馬県',11 => '埼玉県',12 => '千葉県',13 => '東京都',14 => '神奈川県',15 => '新潟県',16 => '富山県',17 => '石川県',18 => '福井県',19 => '山梨県',20 => '長野県',21 => '岐阜県',22 => '静岡県',23 => '愛知県',24 => '三重県',25 => '滋賀県',26 => '京都府',27 => '大阪府',28 => '兵庫県',29 => '奈良県',30 => '和歌山県',31 => '鳥取県',32 => '島根県',33 => '岡山県',34 => '広島県',35 => '山口県',36 => '徳島県',37 => '香川県',38 => '愛媛県',39 => '高知県',40 => '福岡県',41 => '佐賀県',42 => '長崎県',43 => '熊本県',44 => '大分県',45 => '宮崎県',46 => '鹿児島県',47 => '沖縄県'
	);
		$data_rsv['prefecture'] = $m_pref[$data_rsv['pref']];
	}
}
#マイページここまで
      ...
      <div id="mainField">
        ...
        <section id="mainField">
			...
			<ol class="cart_stepbar">
				<li class="visited"><span>1</span><br><div class="newPoint">予約内容入力</div></li>
			...
			<p class="p14">
		  	...
		  	<form name="frm" method="post" action="<?php print($url_currentdir.$filename_current); ?>" id="<?php print($screenid_current); ?>">
					...
					<div class="carttable">
						...
						<table>
							<tr>
								...
								<td>
									<strong style="font-weight:bold;"><?php print(htmlspecialchars($eventname, ENT_QUOTES));?></strong>
								</td>
							</tr>
							<tr>
								<th>
									...
									<ul class="ul-form">
										<li class="clearfix">
											<div class="col clearfix">
												...
												<div class="select">
													<select class="p16" type="select-one" name="mannum" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)">
													<?php
											...
											<div class="col clearfix">
												...
												<div class="select">
													<select class="p16" type="select-one" name="boynum" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)">
													<?php
					...
					<div class="carttable">
						...
						<table>
							<tr>
								...
								<td>
									<input class="form-input p16" type="text" name="lastnamekanji" placeholder="（例）山本" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" style="ime-mode: active"
									<?php
									if (array_key_exists('lastnamekanji', $_POST)) {
										print('value="'.htmlspecialchars($_POST['lastnamekanji'], ENT_QUOTES).'" ');
									}elseif($login_flag){
										echo 'value="'.$data_rsv['sei'].'" ';	
									}
									?>
									size="13" />
									&nbsp;&nbsp;
									<input class="form-input p16" type="text" name="firstnamekanji" placeholder="（例）太郎" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" style="ime-mode: active"
									<?php
									if (array_key_exists('firstnamekanji', $_POST)) {
										print('value="'.htmlspecialchars($_POST['firstnamekanji'], ENT_QUOTES).'" ');
									}elseif($login_flag){
										echo 'value="'.$data_rsv['mei'].'" ';	
									}
									?>
									size="13" />
								</td>
							...
							<tr>
								...
								<td>
									<input class="form-input p16" type="text" name="lastnamekana" placeholder="（例）ヤマモト" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" style="ime-mode: active"
									<?php
									if (array_key_exists('lastnamekana', $_POST)) {
										print('value="'.htmlspecialchars($_POST['lastnamekana'], ENT_QUOTES).'" ');
									}elseif($login_flag){
										echo 'value="'.$data_rsv['sei_kana'].'" ';	
									}
									?>
									size="13" />
									&nbsp;&nbsp;
									<input class="form-input p16" type="text" name="firstnamekana" placeholder="（例）タロウ" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" style="ime-mode: active"
									<?php
									if (array_key_exists('firstnamekana', $_POST)) {
										print('value="'.htmlspecialchars($_POST['firstnamekana'], ENT_QUOTES).'" ');
									}elseif($login_flag){
										echo 'value="'.$data_rsv['mei_kana'].'" ';	
									}
									?>
									size="13" />
								</td>
							...
							<tr>
								...
								<td>
									...
									?>
									<input type="radio" name="gender" value="男性" id="mailmag-man" <?php if ($tmp_gender == "男性") {print('checked="checked" ');}elseif($login_flag == true && $data_rsv['gender'] == 1){ echo 'checked="checked" '; } ?>/>
									<label for="mailmag-man">男性</label>
									<input type="radio" name="gender" id="mailmag-woman" value="女性" <?php if ($tmp_gender != "男性") {print('checked="checked" ');}elseif($login_flag == true && $data_rsv['gender'] == 2){ echo 'checked="checked" '; } ?>/>
									...
									<ul class="ul-form">
										<li class="clearfix">
											<div class="col3 clearfix">
												<div class="select">
													<select class="p16" type="select-one" name="birth_year" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)">
													<?php
														...
														if (array_key_exists('birth_year', $_POST)) {
															...
															print("<option value=\"\">年を選択</option>\n");
														} else {
															$tmp_birth_year = FALSE;
															print("<option value=\"\" selected>年を選択</option>\n");
														}

														foreach ($birth_years as $val) {
															if ($val == $tmp_birth_year) {
																print("<option value=\"".$val."\" selected>".$val."</option>\n");
															} elseif($login_flag == true && $data_rsv['birthday_y'] == $val){ 
											...
											<div class="col3 clearfix">
												<div class="select">
													<select class="p16" type="select-one" name="birth_month" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)">
													<?php
														$birth_months = array();
														for($i=0;$i < 12;$i++){
														...
														if (array_key_exists('birth_month', $_POST)) {
															...
															print("<option value=\"\">月を選択</option>\n");
														} else {
															$tmp_birth_month = FALSE;
															print("<option value=\"\" selected>月を選択</option>\n");
														}

														foreach ($birth_months as $val) {
															if ($val == $tmp_birth_month) {
																print("<option value=\"".$val."\" selected>".$val."</option>\n");
															} elseif($login_flag == true && $data_rsv['birthday_m'] == $val){ 
																echo '<option value="'.$val.'" selected>'.$val.'</option>'."\n"; 
															} else {
																print("<option value=\"".$val."\">".$val."</option>\n");
															}
											...
											<div class="col3 clearfix">
												<div class="select">
													<select class="form-input p16" type="select-one" name="birth_day" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)">
													<?php
														$birth_days = array();
														for($i=0;$i < 31;$i++){
														...
														if (array_key_exists('birth_day', $_POST)) {
															...
															print("<option value=\"\">日を選択</option>\n");
														} else {
															$tmp_birth_day = FALSE;
															print("<option value=\"\" selected>日を選択</option>\n");
														}

														foreach ($birth_days as $val) {
															if ($val == $tmp_birth_day) {
																print("<option value=\"".$val."\" selected>".$val."</option>\n");
															} elseif($login_flag == true && $data_rsv['birthday_d'] == $val){ 
																echo '<option value="'.$val.'" selected>'.$val.'</option>'."\n"; 
															} else {
																print("<option value=\"".$val."\">".$val."</option>\n");
															}
				...
				<h2 class="headline02"><p>
					...
					&nbsp;&nbsp;
							<div style="color:red">
								※J:COMにご登録している「ご住所」「お電話番号」を入力してください
							</div>
					<?php
						} 
					?>
					...
					<div class="carttable">
						...
						<table>
								<tr>
									...
									<td>
										<input class="form-input p16" type="text" id="postalcode1" name="postalcode1" placeholder="000" maxlength="3" onkeyDown="return numOnly(event.keyCode)" onKeyUp="f_checkTabMoveTo(event.keyCode,this, this.form.postalcode2)" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" onChange="zenhan(this);" style="ime-mode: disabled"
										<?php
											if (array_key_exists('postalcode1', $_POST)) {
												print('value="'.htmlspecialchars($_POST['postalcode1'], ENT_QUOTES).'" ');
											}elseif($login_flag){
												echo 'value="'.$data_rsv['zip_1'].'" ';	
											}
										?>
										size="5" />
										...
										<input class="form-input p16" type="text" id="postalcode2" name="postalcode2" placeholder="0000" maxlength="4" onkeyDown="return numOnly(event.keyCode)" onKeyUp="f_checkTabMoveTo(event.keyCode,this, this.form.address1)" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" onChange="zenhan(this);" style="ime-mode: disabled"
										<?php
											if (array_key_exists('postalcode2', $_POST)) {
												print('value="'.htmlspecialchars($_POST['postalcode2'], ENT_QUOTES).'" ');
											}elseif($login_flag){
												echo 'value="'.$data_rsv['zip_2'].'" ';	
											}
										?>
										size="6" />
									</td>
											...
											<li class="clearfix">
												<div class="col2 clearfix">
													<div class="select selectPref">
														<select class="p16" type="select-one" name="address1" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)">
															<?php
																$prefectures =
																...
																foreach ($prefectures as $val) {
																	if ($val == $tmp_address1) {
																		print("<option value=\"".$val."\" selected>".$val."</option>\n");
																	} elseif($login_flag == true && $data_rsv['prefecture'] == $val){ 
																		echo '<option value="'.$val.'" selected>'.$val.'</option>'."\n"; 
																	} else {
																		print("<option value=\"".$val."\">".$val."</option>\n");
																	}
								...
								<tr>
									...
									<td>
										<input class="form-input p16" type="text" name="address2" placeholder="（例）中央区" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" style="ime-mode: active;width:250px;"
										<?php
											if (array_key_exists('address2', $_POST)) {
												print('value="'.htmlspecialchars($_POST['address2'], ENT_QUOTES).'" ');
											}elseif($login_flag){
												echo 'value="'.$data_rsv['address1'].'" ';	
											}
										?>
									   	 />
									</td>
								...
								<tr>
									...
									<td>
										<input class="form-input p16" type="text" name="address3" placeholder="（例）箱崎町20-1　ポケカルマンション5階" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" style="ime-mode: active;width:410px;"
											<?php
												if (array_key_exists('address3', $_POST)) {
													print('value="'.htmlspecialchars($_POST['address3'], ENT_QUOTES).'" ');
												}elseif($login_flag){
													echo 'value="'.$data_rsv['address2'].$data_rsv['address3'].'" ';	
												}
											?>	
										size="40" />
									</td>
								...
								<tr>
									...
									<td>
										<input class="form-input p16" type="text" id="telephone1" name="telephone1" placeholder="0000" maxlength="4" onkeyDown="return numOnly(event.keyCode)" onKeyUp="f_checkTabMoveTo(event.keyCode,this, this.form.telephone2);"  onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" onchange="zenhan(this);" style="ime-mode: disabled"
										<?php
											if (array_key_exists('telephone1', $_POST)) {
												print('value="'.htmlspecialchars($_POST['telephone1'], ENT_QUOTES).'" ');
											}elseif($login_flag){
												echo 'value="'.$data_rsv['tel1_1'].'" ';	
											}
										?>
										size="6" />
										-
										<input class="form-input p16" type="text" id="telephone2" name="telephone2" placeholder="0000" maxlength="4" onkeyDown="return numOnly(event.keyCode)" onKeyUp="f_checkTabMoveTo(event.keyCode,this, this.form.telephone3)"  onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" onchange="zenhan(this);" style="ime-mode: disabled"
										<?php
											if (array_key_exists('telephone2', $_POST)) {
												print('value="'.htmlspecialchars($_POST['telephone2'], ENT_QUOTES).'" ');
											}elseif($login_flag){
												echo 'value="'.$data_rsv['tel1_2'].'" ';	
											}
										?>
										size="6" />
										-
										<input class="form-input p16" type="text" id="telephone3" name="telephone3" placeholder="0000" maxlength="4"  onkeyDown="return numOnly(event.keyCode)" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" onchange="zenhan(this);" style="ime-mode: disabled"
										<?php
											if (array_key_exists('telephone3', $_POST)) {
												print('value="'.htmlspecialchars($_POST['telephone3'], ENT_QUOTES).'" ');
											}elseif($login_flag){
												echo 'value="'.$data_rsv['tel1_3'].'" ';	
											}
										?>
										size="6" />
										<?php
											if ($_SESSION['salesforcenolinkflg'] == SF_NOLINK_FLAG_NOLINK) {
										?>
												<span class="p14 x-small"><strong>※携帯電話の番号を半角数字でご入力ください。</strong></span>
										<?php
											} else {
										?>
										<span class="p14">※自宅電話か携帯電話のどちらかの番号を半角数字でご入力ください。</span>
										<?php
											}
										?>
									</td>
								...
								<tr>
									...
									<td>
										<input class="form-input p16" type="text" name="mailaddress" placeholder="（例）abc@xxxx.com" onFocus="f_onFocus(this)" onBlur="f_onBlur(this)" style="ime-mode: disabled;width:380px;"
										<?php
											if (array_key_exists('mailaddress', $_POST)) {
												print('value="'.htmlspecialchars($_POST['mailaddress'], ENT_QUOTES).'" ');
											}elseif($login_flag){
												echo 'value="'.$data_rsv['email'].'" ';	
											}
										?>
										size="40" />
									</td>
								...
								<tr class="last">
									<td class="single" colspan="2">
										<dl class="cart_attention">
											<dt>※</dt>
											<dd>
												Eメールアドレスについては、入力間違いがないようにお願いします。誤って入力された場合、弊社からの予約完了メールや契約成立メールの受け取りができません。また、ドメイン指定受信をされている場合、「＠poke.co.jp」からのメール受信ができるよう設定ください。
											</dd>
										</dl>
									</td>
								</tr>
							...
							<tr class="last">
								...
								<td>
									...
									?>
											...
											<label for="paymethod-card">現地支払い</label>
									<?php
										}else{
											//顧客の支払先が「ポケカル」→通常の決済処理
									...
									?>
											<input type="radio" name="paymethod" value="card" id="paymethod-card" <?php if ($tmp_paymethod == "card") {print('checked="checked" ');}?>/>
											...
											?>
											<input type="radio" name="paymethod" id="paymethod-discard" value="discard" <?php if ($tmp_paymethod != "card") {print('checked="checked" ');}?>/>
											<label for="paymethod-discard">払込票</label>
											<?php
													}else{
											?>
											<input type="radio" name="paymethod" id="paymethod-discard" value="" disabled />
											<label for="paymethod-discard"><font color="Silver">払込票</font></label><br />
											<?php
													}
											?>
											<?php
                                            ...
                                            <p p14><strong>※当商品のお支払いは、クレジットカードのみとなります。</strong></p>
											...
											?>
												<span class="p14">
													<p>※クレジットカード決済ご希望の方は、カード会社の決済画面に移動します。</p>
													<p>　カード情報も入力して頂きますが、ご参加決定後の決済となりますのでご安心くださいませ。</p>
												</span>
											<?php
											}
										}
				...
				<div class="panel">
					<div class="carttable">
						...
						<table class="horizon">
							<td>
								個人情報の取り扱いについて<br>
								株式会社ポケカル(以下、｢当社」といいます。）は、個人情報(お客様の氏名、住所、電話番号、生年月日、性別など特定の個人を識別できるもの、以下も同様とします。）を取扱う際に、個人情報保護に関する諸法令、および主務大臣のガイドラインに定められた義務、並びに本ポリシーを遵守します。  <a href="/privacy.html" target="_blank">詳しくはこちら≫</a><br>
								<?php
								  if($isJCOM) {
								?>
									<br />お預かりした個人情報は、加入者確認のために（株）ジュピターテレコムに開示いたします。
									...
									?>
									<br />
									<input type="radio" name="personalinfo" value="ok" id="personalinfo-ok" <?php if ($tmp_personalinfo == "ok") {print('checked="checked" ');}?>/>
									<label for="personalinfo-ok">同意する</label>　　　
									<input type="radio" name="personalinfo" id="personalinfo-ng" value="ng" <?php if ($tmp_personalinfo != "ok") {print('checked="checked" ');}?>/>
									<label for="personalinfo-ng">同意しない</label>
								<?php
								  }
								?>  
								<hr />
								割引・<?php if($isJCOM) { ?>新ツアー<?php }else{ ?>新企画<?php } ?>情報を　　
								<?php
								...
								?>
								<input type="radio" name="mailmag" value="ok" id="mailmag-ok" <?php if ($tmp_mailmag == "ok") {print('checked="checked" ');}?>/>
								<label for="mailmag-ok">もらう</label>　　　
								<input type="radio" name="mailmag" id="mailmag-ng" value="ng" <?php if ($tmp_mailmag != "ok") {print('checked="checked" ');}?>/>
								<label for="mailmag-ng">もらわない</label><br>
								...
								*購読の解除はトップページのメルマガ専用フォームより行えます。
								...
								?>
									<hr />
									<p>毎月発行「ポケカル」情報誌を　<input type="radio" name="documentflg" value="ok" id="documentflg-ok" <?php if ($tmp_documentflg == "ok") {print('checked="checked" ');}?>/>
									<label for="documentflg-ok">発送希望</label>　　　
									<input type="radio" name="documentflg" id="documentflg-ng" value="ng" <?php if ($tmp_documentflg != "ok") {print('checked="checked" ');}?>/>
									<label for="documentflg-ng">発送不要</label>
									</p>
									<p>*ポケカルが毎月発行しているツアーカタログ「ポケカル日帰り遊び」を無料でお送りいたします。</p>
									<p>*発送解除はお電話・メール・FAXにて行えます。</p>
								<?php
								  }
								?> 								
							</td>
	  			...
	  			<p class="btn-detail"><a href="javascript:void(0);" onclick="document.frm.submit();">確認画面へ</a></p>
...
$(document).ready(function() {
  ...
  if($('input[name="personalinfo"]:checked').val() === "ng"){
    //$('#confirmButton').removeClass('confirmButton').addClass('confirmButtonOff');
	$('#confirmButton').removeClass('submit_btn').addClass('submit_btn');
    $('#confirmButton').attr('disabled', true);
  }else{
    //$('#confirmButton').removeClass('confirmButtonOff').addClass('confirmButton');
	$('#confirmButton').removeClass('submit_btn').addClass('submit_btn');
  ...
  $('input[name="personalinfo"]:radio').live('change', function() {
    if($(this).val() === "ng"){
      //$('.confirmButton').css('background', 'url("/images/book/button_confirm_off.jpg") no-repeat scroll left top transparent;');
      //$('#confirmButton').removeClass('confirmButton').addClass('confirmButtonOff');
	  $('#confirmButton').removeClass('submit_btn').addClass('submit_btn');
      $('#confirmButton').attr('disabled', true);
    }else{
...
<?php
	...
	echo '$_SESSION[\'firstvisitday\']='.$_SESSION['firstvisitday']."<br />";
        ...
        echo '$_SESSION[\'eventid\']='.$_SESSION['eventid']."<br />";
    }
?>
